...
class ProductController extends Controller
{
    ...
    public function index(Request $request, $subdomain)
    {
        try {
            ...
            return view('contractor.admin.product.index', ['products' => $products, "subdomain" => $subdomain, 'contractor' => $contractor, 'productgroup' => $productgroup, 'setting'=>$setting]);
        } catch (\Exception $e) {
    ...
    public function update_selection(Request $request, $subdomain, $product)
    {
        try {
            ...
            $validator->sometimes('product_price', 'nullable|numeric', function ($input) {
            ...
            $product->contractors()->detach($request->contractor_details->id);

            if ($request["status"] == 1) {
                $validated['product_group_id'] = $product->pdt_group_id;

                $product->contractors()->attach([$request->contractor_details->id => $validated]);
            }
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }

        if($request['price_updated']) {
            $message = 'Product Price has been updated successfully';
        } else {
            $message = 'Product has been ' . ($request['status'] == 1 ? 'selected' : 'deselected') . ' successfully';
        }
