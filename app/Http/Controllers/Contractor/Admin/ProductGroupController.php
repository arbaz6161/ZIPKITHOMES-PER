...
class ProductGroupController extends Controller
{
    ...
    public function index(Request $request, $subdomain)
    {
        try {
            ...
            return view('contractor.admin.productgroup.index', ['productgroups' => $productgroups, "subdomain" => $subdomain, "contractor" => $request->contractor_details, 'setting'=> $setting]);
        } catch (\Exception $e) {
    ...
    public function update_selection(Request $request, $subdomain, $productgroup)
    {
        try {
            ...
            if ($values["status"] == 1)
                $productgroup->contractors()->attach([$request->contractor_details->id]);
            else
                $productgroup->contractors()->detach($request->contractor_details->id);
        } catch (\Exception $e) {
            return back()->with('error', $e->getMessage());
        }
